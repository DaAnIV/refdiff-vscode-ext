...
export class CodeSimiliarity {
  ...
  constructor(before: CST, after: CST,
  ...
  private addNode(
    ...
    for (let token of tokenizedFile.tokens) {
      ...
      let tokenStr = fileContent.toString('ascii', token.start, token.end);
  ...
  public sim(node1: CSTNode, node2: CSTNode): number {
    ...
    return CodeSimiliarity.calculateWJC(this.idfPerToken, multiset1, multiset2);
  }
  ...
  public extractedSim(
    ...
    let afterMultiset = this.nodesBodyTokens.get(afterNode) as Multiset<string>;
  ...
  private static calculateWJC(
    ...
    idfMap.forEach((idf, token) => {
      ...
      bottom += Math.max(multiset1.get(token), multiset2.get(token)) * idf;
    });
